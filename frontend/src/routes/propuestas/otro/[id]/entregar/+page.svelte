<script lang="ts">
	...
	async function loadData() {
		...
		try {
			const [polizaData, aseguradorasData] = await Promise.all([
				polizaService.otroBien.getById(polizaId),
	...
	async function handleAseguradoraChange(aseguradoraNumero: number) {
		const idAseguradoraKey = `id_aseguradora_${aseguradoraNumero}` as keyof PolizaOtroBien;
		...
		try {
			...
			const valorPrimaKey = `valor_prima_aseg_${aseguradoraNumero}` as keyof PolizaOtroBien;
	...
	async function handleSubmit(event: Event) {
		...
		try {
			...
			const result = await polizaService.otroBien.entregar(polizaId, payload);
