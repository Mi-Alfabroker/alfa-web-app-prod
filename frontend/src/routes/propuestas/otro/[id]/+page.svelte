<script lang="ts">
	...
	import type { PolizaOtroBien, EstadoPoliza, CambiarEstadoDto } from '$lib/types/poliza';
	...
	function initFormData() {
		...
		formData = {
			...
			valor_bien_asegurado: poliza.valor_bien_asegurado || undefined,
	...
	async function handleSave() {
		...
		try {
			...
			await polizaService.otroBien.update(polizaId, dataToSend);
			addNotification({ type: 'success', title: 'Guardado', message: 'Los cambios se han guardado correctamente' });
			editMode = false;
			await loadData();
		} catch (err) {
			console.error('Error saving:', err);
			addNotification({ type: 'error', title: 'Error', message: err instanceof Error ? err.message : 'No se pudieron guardar los cambios' });
		} finally {
			saving = false;
		}
	}
	...
	async function handleCambiarEstado() {
		...
		try {
			...
			await polizaService.otroBien.cambiarEstado(polizaId, data);
			addNotification({ type: 'success', title: 'Estado actualizado', message: `La póliza ahora está en estado ${nuevoEstado}` });
			showEstadoModal = false;
			await loadData();
		} catch (err) {
			console.error('Error changing status:', err);
			addNotification({ type: 'error', title: 'Error', message: err instanceof Error ? err.message : 'No se pudo cambiar el estado' });
		} finally {
			saving = false;
		}
	}
	...
	function getClienteName(): string {
		if (!cliente) return '—';
		return cliente.tipo_persona === 'PERSONA' ? cliente.nombre || '—' : cliente.razon_social || '—';
	}
...
<header class="page-header">
	<div class="flex items-center gap-4">
		<button type="button" class="p-2 hover:bg-secondary-100 rounded-lg transition-colors" on:click={() => goto('/propuestas')}>
			<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
				<path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7" />
			</svg>
		</button>
		<div>
			<h1 class="page-title">{poliza?.estado === 'PROSPECTO' ? 'Propuesta' : 'Póliza'} Otro Bien</h1>
			{#if poliza}<p class="text-secondary-500 text-sm mt-1 font-mono">{poliza.consecutivo}</p>{/if}
		</div>
	</div>
	...
	{#if poliza}
		<div class="flex items-center gap-3">
			...
			{#if poliza.estado !== 'CANCELADA'}
				<button type="button" class="btn btn-secondary" on:click={openCambiarEstadoModal}>Cambiar Estado</button>
			{/if}
			...
			{#if poliza.estado === 'PROSPECTO'}
				<button
					...
					on:click={() => goto(`/propuestas/otro/${polizaId}/generar`)}
				>
			...
			{#if poliza.estado === 'PROSPECTO' && (poliza.valor_prima_aseg_1 || poliza.valor_prima_aseg_2 || poliza.valor_prima_aseg_3)}
				<button
					type="button"
					...
					on:click={() => goto(`/propuestas/otro/${polizaId}/entregar`)}
				>
					<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
			...
			{#if !editMode}
				<button type="button" class="btn btn-primary" on:click={() => editMode = true}>Editar</button>
			{/if}
		</div>
	{/if}
</header>
	...
	{:else if poliza}
		...
		<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
			<div class="lg:col-span-2 space-y-6">
					...
					{#if otroBien}
						<div class="grid grid-cols-2 gap-4">
							...
							<div class="col-span-2">
								...
								<p class="text-sm">{otroBien.bien_asegurado || '—'}</p>
							</div>
							<div class="col-span-2">
								<p class="text-xs text-secondary-500 mb-1">Detalles</p>
								<p class="text-sm whitespace-pre-wrap">{otroBien.detalles_bien_asegurado || '—'}</p>
							</div>
						</div>
					{/if}
				</div>
				...
				<div class="card">
					<h2 class="text-lg font-semibold text-secondary-900 mb-4">Valor Asegurado</h2>
					{#if editMode}
						<FormField label="Valor del Bien Asegurado">
							<CurrencyInput bind:value={formData.valor_bien_asegurado} />
						</FormField>
					{:else}
						<div class="flex justify-between py-2">
							<span class="text-secondary-600">Valor del Bien</span>
							<span class="font-medium text-primary-600">{formatCurrency(poliza.valor_bien_asegurado)}</span>
						</div>
					{/if}
				</div>
