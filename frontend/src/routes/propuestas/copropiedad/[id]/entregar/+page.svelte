<script lang="ts">
	...
	async function loadData() {
		...
		try {
			const [polizaData, aseguradorasData] = await Promise.all([
				polizaService.copropiedad.getById(polizaId),
			...
			if (poliza.estado !== 'PROSPECTO') {
				...
				goto(`/propuestas/copropiedad/${polizaId}`);
				return;
			}
	...
	async function handleAseguradoraChange(aseguradoraNumero: number) {
		const idAseguradoraKey = `id_aseguradora_${aseguradoraNumero}` as keyof PolizaCopropiedad;
		...
		try {
			...
			const valorPrimaKey = `valor_prima_aseg_${aseguradoraNumero}` as keyof PolizaCopropiedad;
	...
	async function handleSubmit(event: Event) {
		...
		try {
			...
			const result = await polizaService.copropiedad.entregar(polizaId, payload);
	...
	<div class="flex items-center gap-4">
		...
		<div>
			...
			<p class="text-sm text-secondary-600">
				{#if poliza}
					{poliza.consecutivo} - Copropiedad
				{/if}
			</p>
		</div>
	</div>
