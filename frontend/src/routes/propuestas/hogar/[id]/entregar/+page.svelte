<script lang="ts">
	...
	let formData: Record<string, any> = {
		...
		valor_comision: 0,
		// Financiación (solo si medio_pago = 'financiera')
	...
	async function loadData() {
		...
		error = null;
		try {
			// Cargar póliza y aseguradoras
			const [polizaData, aseguradorasData] = await Promise.all([
				polizaService.hogar.getById(polizaId),
			...
			// Validar que la póliza sea PROSPECTO
			if (poliza.estado !== 'PROSPECTO') {
				...
				goto(`/propuestas/hogar/${polizaId}`);
				return;
			}

			// Pre-llenar fechas de vigencia si existen
	...
	async function handleAseguradoraChange(aseguradoraNumero: number) {
		// Obtener ID de aseguradora según el número (1-5)
		const idAseguradoraKey = `id_aseguradora_${aseguradoraNumero}` as keyof PolizaHogar;
		...
		try {
			// Llamar al endpoint de datos-entrega para auto-rellenar
			...
			// Pre-llenar valor prima seleccionada
			const valorPrimaKey = `valor_prima_aseg_${aseguradoraNumero}` as keyof PolizaHogar;
	...
	async function handleSubmit(event: Event) {
		...
		try {
			...
			// Preparar payload (excluir aseguradora_numero que solo es UI)
			const payload = {
			...
			console.log('📤 Entregando póliza:', payload);

			// Llamar endpoint de entrega
			const result = await polizaService.hogar.entregar(polizaId, payload);

			console.log('✅ Póliza entregada:', result);
...
<main class="page-content">
	...
	{:else if poliza}
		<form on:submit={handleSubmit} class="max-w-5xl mx-auto">
			...
			<!-- Error Message -->
			{#if error}
				<div class="mx-8 mb-6 p-4 bg-red-50 border border-red-200 rounded-lg">
