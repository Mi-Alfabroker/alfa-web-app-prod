"""Business logic service for PolizaCopropiedad (Co-ownership Insurance Policy) operations."""
...
from app.repositories.polizas import poliza_copropiedad_repository
from app.repositories.bienes.copropiedad_repository import get_by_id as copropiedad_get_by_id
from app.repositories.aseguradora_repository import AseguradoraRepository


class PolizaCopropiedadService:
    """
    Business Logic Layer for PolizaCopropiedad operations.
    ...
    def get_all() -> List[dict]:
        ...
        polizas = poliza_copropiedad_repository.get_all()
        return [p.to_dict() for p in polizas]

    @staticmethod
    def get_by_id(poliza_id: int) -> Optional[dict]:
        """Get a co-ownership insurance policy by ID."""
        poliza = poliza_copropiedad_repository.get_by_id(poliza_id)
    ...
    def get_by_consecutivo(consecutivo: str) -> Optional[dict]:
        ...
        poliza = poliza_copropiedad_repository.get_by_consecutivo(consecutivo)
        return poliza.to_dict() if poliza else None

    @staticmethod
    def get_by_copropiedad_id(copropiedad_id: int) -> List[dict]:
        """Get all policies for a specific co-ownership asset."""
        polizas = poliza_copropiedad_repository.get_by_bien_id(copropiedad_id)
        return [p.to_dict() for p in polizas]

    @staticmethod
    def get_by_usuario_id(usuario_id: int) -> List[dict]:
        """Get all co-ownership policies for a specific user/client."""
        polizas = poliza_copropiedad_repository.get_by_usuario_id(usuario_id)
        return [p.to_dict() for p in polizas]

    @staticmethod
    def get_by_estado(estado: str) -> List[dict]:
        """Get all policies with a specific state."""
        polizas = poliza_copropiedad_repository.get_by_estado(estado)
        return [p.to_dict() for p in polizas]

    @staticmethod
    def get_por_vencer(dias: int = 30) -> List[dict]:
        """Get policies that will expire within the specified days."""
        polizas = poliza_copropiedad_repository.get_por_vencer(dias)
        return [p.to_dict() for p in polizas]
    ...
    def create(data: dict) -> tuple[dict, Optional[str]]:
        ...
        Args:
            data: Dictionary with policy fields including id_copropiedad
            
        Returns:
            Tuple of (created policy dict, error message or None)
        """
        # Validate required fields
        if not data.get('id_copropiedad'):
            return {}, "Field 'id_copropiedad' is required"
        
        # Validate co-ownership asset exists
        copropiedad = copropiedad_get_by_id(data['id_copropiedad'])
        if not copropiedad:
            return {}, f"Copropiedad with ID {data['id_copropiedad']} not found"
        
        # Get client ID from the co-ownership asset
        cliente_id = copropiedad.id_usuario
        ...
        if not data.get('consecutivo'):
            ...
            counter = 1
            while poliza_copropiedad_repository.exists_consecutivo(consecutivo):
        ...
        poliza = poliza_copropiedad_repository.create(data)
    ...
    def update(poliza_id: int, data: dict) -> tuple[dict, Optional[str]]:
        ...
        poliza = poliza_copropiedad_repository.get_by_id(poliza_id)
        if not poliza:
            return {}, f"PolizaCopropiedad with ID {poliza_id} not found"
        ...
        updated = poliza_copropiedad_repository.update(poliza, data)
    ...
    def cambiar_estado(poliza_id: int, nuevo_estado: str, aseguradora_seleccionada: Optional[int] = None) -> tuple[dict, Optional[str]]:
        ...
        updated = poliza_copropiedad_repository.update_estado(poliza, nuevo_estado)
        return updated.to_dict(), None
    ...
    def validar_valores_asegurados(poliza_id: int) -> tuple[bool, List[str]]:
        ...
        poliza = poliza_copropiedad_repository.get_by_id(poliza_id)
        if not poliza:
            return False, [f"PolizaCopropiedad with ID {poliza_id} not found"]
        
        copropiedad = copropiedad_get_by_id(poliza.id_copropiedad)
        if not copropiedad:
            return False, ["Associated Copropiedad not found"]
        
        errores = poliza.validar_valores_asegurados(copropiedad)
    ...
    def entregar_poliza(poliza_id: int, datos_entrega: dict) -> tuple[dict, Optional[str]]:
        ...
        # Update policy
        updated = poliza_copropiedad_repository.update(poliza, datos_entrega)
        return updated.to_dict(), None
    ...
    def actualizar_entrega(poliza_id: int, datos_actualizacion: dict) -> tuple[dict, Optional[str]]:
        ...
        # Update policy
        updated = poliza_copropiedad_repository.update(poliza, datos_actualizacion)
        return updated.to_dict(), None
    ...
    def delete(poliza_id: int) -> tuple[bool, Optional[str]]:
        ...
        poliza_copropiedad_repository.delete(poliza)
        return True, None
